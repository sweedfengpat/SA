...
func (e ConfirmationEdges) OwnerOrErr() (*User, error) {
	if e.loadedTypes[0] {
		if e.Owner == nil {
...
func (e ConfirmationEdges) BillOrErr() ([]*Bill, error) {
	if e.loadedTypes[1] {
		return e.Bill, nil
	}
...
// QueryOwner queries the owner edge of the Confirmation.
func (c *Confirmation) QueryOwner() *UserQuery {
	return (&ConfirmationClient{config: c.config}).QueryOwner(c)
}

// QueryBill queries the bill edge of the Confirmation.
func (c *Confirmation) QueryBill() *BillQuery {
	return (&ConfirmationClient{config: c.config}).QueryBill(c)
}
