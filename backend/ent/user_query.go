...
func (uq *UserQuery) QueryConfirmation() *ConfirmationQuery {
	query := &ConfirmationQuery{config: uq.config}
...
func (uq *UserQuery) QueryBill() *BillQuery {
...
func (uq *UserQuery) WithConfirmation(opts ...func(*ConfirmationQuery)) *UserQuery {
	query := &ConfirmationQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withConfirmation = query
	return uq
}
...
func (uq *UserQuery) WithBill(opts ...func(*BillQuery)) *UserQuery {
	query := &BillQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withConfirmation; query != nil {
		...
		query.Where(predicate.Confirmation(func(s *sql.Selector) {
			s.Where(sql.InValues(user.ConfirmationColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.Confirmation = append(node.Edges.Confirmation, n)
		}
	}
