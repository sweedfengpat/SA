...
func (cq *ConfirmationQuery) QueryOwner() *UserQuery {
...
func (cq *ConfirmationQuery) WithOwner(opts ...func(*UserQuery)) *ConfirmationQuery {
	query := &UserQuery{config: cq.config}
...
func (cq *ConfirmationQuery) WithBill(opts ...func(*BillQuery)) *ConfirmationQuery {
	query := &BillQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *ConfirmationQuery) sqlAll(ctx context.Context) ([]*Confirmation, error) {
	...
	if query := cq.withBill; query != nil {
		...
		query.Where(predicate.Bill(func(s *sql.Selector) {
			s.Where(sql.InValues(confirmation.BillColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.confirmation_id
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "confirmation_id" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "confirmation_id" returned %v for node %v`, *fk, n.ID)
			}
